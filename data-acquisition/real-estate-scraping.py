...
colum_name = ['id', 'type', 'subtype', 'price', 'transactionType', 'zip', 'visualisationOption', 'kitchen', 'building',
              'energy', 'certificates', 'bedroom', 'land', 'atticExists', 'basementExists', 'outdoor', 'specificities', 'wellnessEquipment', 'parking', 'condition', 'location']
file_name = 'scrape_immoweb.csv'
with open(file_name, 'w', newline='') as csvfile:
    writer = csv.DictWriter(csvfile, delimiter='\t', fieldnames=colum_name)
    writer.writeheader()
    writer.writerow(get_house_dict())
