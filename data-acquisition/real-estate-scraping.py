...
for page in range(1, 334):
    ...
    with open('immowebprovince.csv', 'a', newline="", encoding='utf-8') as f:
        ...
        for i in range(30):
            ...
            for key in keys:
                if key == 'region':
                    dict_house[key].append(house_dict[0]['property']['location']['region'])
                elif key == 'province':
                    dict_house[key].append(house_dict[0]['property']['location']['province'])
                elif key == 'EPC':
                    dict_house[key].append(house_dict[0]['transaction']['certificate'])
            ...
            def scrape(link):
                ...
                script_text = property_soup.find('script', text=re.compile("\s+window.dataLayer")).text.split('= ', 1)[
                    1]
